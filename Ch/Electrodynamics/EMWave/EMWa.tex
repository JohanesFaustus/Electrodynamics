...
\end{equation*}
approaches from the left, giving rise to a reﬂected wave,
\begin{equation*}
...
We then define the Brewster angle $\theta_B$  at which the reﬂected wave is completely extinguished $E_R=0$.
...
\end{equation*}
For the typical case $\mu_1 \approx\mu_2$, so "$\beta\approx n_2/n_1$ and
\begin{align*}
    \sin^2 \theta_B & = \frac{1 - \beta^2}{(n_1/n_2)^2 - \beta^2}
...
\end{equation*}
while the reﬂected and transmitted intensities are
\begin{equation*}
    I_I=\frac{1 }{2}\epsilon_Iv_IE_{0I}^2\cos \theta_I
    I_I=\frac{1 }{2}\epsilon_Iv_IE_{0I}^2\cos \theta_I
\end{equation*}

The reﬂection and transmission coefﬁcients for waves polarized parallel to the plane of incidence are
\begin{align*}
