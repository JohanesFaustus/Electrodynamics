...
\end{equation*}
which can be written using Gauss's law $\rho=\epsilon_0 \nabla \cdot \mathbf{E}$ then integration by parts $    \int_{a}^{b} f(\frac{dg}{dx})\;dx=-\int_{a}^{b} g\left(\frac{df}{dx}\right)\;dx+ fg \lvert_{a}^{b}$ as
\begin{align*}
    W & =  \frac{\epsilon_0}{2} \int (\nabla \cdot \mathbf{E}) \mathbf{V} \, d\tau                                                  \\
      & = \frac{\varepsilon_0}{2} \left[ - \int \mathbf{E} \cdot (\nabla V)\; d\tau + \oint V \mathbf{E} \cdot d \mathbf{a} \right] \\
    W & = \frac{\epsilon_0}{2}\Biggl(\int_{V}E^2\;d\tau+ \oint V\mathbf{E}\cdot d\mathbf{a}\Biggr)
\end{align*}
where we have transferred the derivative from $\mathbf{E}$ to $V$.
Integration can be done over whatever volume you use (as long as it encloses all the charge), but the contribution from the volume integral goes up (due to $d\tau$), and that of the surface integral goes down (due to $\mathbf{E}$ and $V$), as you take larger and larger volumes.
Thus,
\begin{equation*}
