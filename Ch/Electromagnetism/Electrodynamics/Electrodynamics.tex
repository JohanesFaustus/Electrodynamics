...
\end{equation*}
If we start with zero current and build it up to a ﬁnal value I, the work done is obtained integrating the last equation over time
\begin{equation*}
    W=\frac{1}{2}LI^2
\end{equation*}
Recall
\begin{equation*}
    \Phi = \int B \cdot d \mathbf{a} = \int (\nabla \times \mathbf{A}) \cdot d \mathbf{a} = \oint \mathbf{A} \cdot d \mathbf{l}
\end{equation*}
Since $\Phi=LI$, then
\begin{equation*}
...
Therefore
\begin{equation*}
    W = \frac{1}{2} \mathbf{I} \oint \mathbf{A} \cdot d \mathbf{l} = \frac{1}{2} \oint (\mathbf{A} \cdot \mathbf{I}) dl \implies \frac{1}{2} \int_\mathcal{V} (\mathbf{A} \cdot \mathbf{J}) \;d\tau
\end{equation*}
The generalization comes from the definition of current density $    \mathbf{J}\;d\tau = \mathbf{I}\;d \mathbf{l}$.
Next use Ampere law $\nabla  \times \mathbf{B}=\mu_0 \mathbf{J}$
\begin{equation*}
    W  = \frac{1}{2} \int_\mathcal{V} \mathbf{A} \cdot \nabla \times \mathbf{B} \;d\tau
\end{equation*}
Then use integration by parts $    \int_{a}^{b} f(\frac{dg}{dx})\;dx=-\int_{a}^{b} g\left(\frac{df}{dx}\right)\;dx+ fg \lvert_{a}^{b}$ to transfer the derivative from $\mathbf{B}$ to $\mathbf{A}$
\begin{align*}
    W & = \frac{1}{2\mu_0}\bigg[\int_\mathcal{V} B^2\;d\tau-\int \nabla \cdot ( \mathbf{A}\times \mathbf{B}) \;d\tau\bigg] \\
    W & = \frac{1}{2\mu_0}\bigg[\int_\mathcal{V} B^2\;d\tau-\oint \mathbf{A}\times \mathbf{B}\cdot d\mathbf{a}\bigg]
\end{align*}
Now, the integration is to be taken over the entire volume occupied by the current, but \textbf{J} is zero out there anyway. In particular, if we agree to integrate over all space, then the surface integral goes to zero (due to $\mathbf{A}$ and $\mathbf{B}$), and we are left with
\begin{equation*}
...
In terms of free charges and currents, Maxwell’s equations read
\begin{align*}
...
\end{align*}
and their integral form
\begin{align*}
    \oint_{\mathcal{S}} \mathbf{D} \cdot d\mathbf{A} & = Q_{f_\text{enc}}                                                                          \\
    \oint_{\mathcal{S}} \mathbf{B} \cdot d\mathbf{A} & = 0,                                                                                        \\
    \oint_{\mathcal{P}} \mathbf{E} \cdot d\mathbf{l} & = -\frac{d}{dt} \int_S \mathbf{B} \cdot d\mathbf{A},                                        \\
    \oint_{\mathcal{P}} \mathbf{H} \cdot d\mathbf{l} & = \int_S \mathbf{J}_f \cdot d\mathbf{A} + \frac{d}{dt} \int_S \mathbf{D} \cdot d\mathbf{A}.
\end{align*}
where $\mathcal{S}$ denote integration over any closed surface $\mathcal{S}$ and $\mathcal{P }$ denote integration over any surface $\mathcal{S }$ bounded by the closed loop $\mathcal{P}$.

\subsubsection{Boundary Conditions}
In the case of linear media, the boundary conditions can be expressed in terms of $\mathbf{E}$ and $\mathbf{B}$ alone
\begin{align*}
    \epsilon_1 E_1^{\perp} - \epsilon_2 E_2^{\perp}                                     & =  \sigma_f                             \\
    \mathbf{B}_1^{\perp} - \mathbf{B}_2^{\perp}                                         & =  0                                    \\
    \mathbf{E}_1^{\parallel} - \mathbf{E}_2^{\parallel}                                 & =  0                                    \\
    \frac{1}{\mu_1} \mathbf{B}_1^{\parallel} - \frac{1}{\mu_2} \mathbf{B}_2^{\parallel} & =  \mathbf{K}_f \times \hat{\mathbf{n}} \\
\end{align*}
where $\parallel$ denote tangential components and $\perp$ denote normal components.
\end{document}